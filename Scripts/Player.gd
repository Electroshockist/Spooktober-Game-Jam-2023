...
@export var camera_clamps: Vector2 = Vector2(30, -85)
...
func _input(event):	
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:		
		self.rotate_y(-event.relative.x * rotation_sensitvity * 0.001)
		
		#print(gimbal.rotation.x)
		#gimbal.rotation.x = clampf(gimbal.rotation.x + event.relative.y * rotation_sensitvity * 0.001, camera_clamps.y, camera_clamps.x)
		gimbal.rotate_x(event.relative.y * rotation_sensitvity * 0.001)
		print(gimbal.get)
